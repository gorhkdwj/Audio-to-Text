...
### T-007 · GUI에서 "HuggingFace 토큰이 없습니다" (환경변수 스냅숏 문제)
**발생 상황**
- 사용자가 GUI 첫 실사용(화자 구분 체크, 2026-07-04) 중 보고. 텍스트 변환 자체는 D-003 폴백으로 정상 완료(안내 후 계속).

**증상**
- 토큰을 사용자 레지스트리에 등록했는데도 `[안내] HuggingFace 토큰이 없습니다.` 출력, 화자 구분 생략.

**확인된 원인**
- Windows 자식 프로세스는 **부모 프로세스의 환경변수 스냅숏**을 물려받는다(레지스트리를 다시 읽지 않음).
- 해당 GUI 인스턴스는 토큰 등록 **이전에 시작된** 개발 세션에서 실행돼, GUI → QProcess(CLI)까지 HF_TOKEN 없는 옛 환경이 전파됨.
- 개발 중 CLI 테스트는 매번 레지스트리에서 토큰을 수동 주입해 통과했기 때문에 드러나지 않았다.

**조치**
- diarizer에 `find_hf_token()` 도입: **명시 인자 → 환경변수 → HKCU\Environment(사용자 레지스트리)** 3단계 탐색.
- 변환은 하위 프로세스가 코드를 새로 읽으므로 떠 있는 GUI 재시작 불필요.
- 검증: 환경변수 없는 셸에서 `--diarize` 실행 → 화자 1/2 라벨 정상, 안내 없음. 토큰 탐색 단위 테스트 4건 추가.

**재발 방지**
- 사용자 설정값은 프로세스 환경만 믿지 말고 **원본 저장소(레지스트리)까지 탐색**한다.
- 실행 경로(GUI/CLI/바로가기/예약 작업)마다 환경 상속이 다름을 전제하고 설계한다.
