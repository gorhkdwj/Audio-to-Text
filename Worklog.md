...
### W-017 · S5 문서 최종 정리 및 프로젝트 마무리
**요청**
- "마무리해줘".

**수행 작업**
- 전체 단위 테스트 최종 실행(38/38 통과). 추적 파일 목록 점검: 개인 콘텐츠(mp4·out·tests-output) 미추적 확인.
- README 보강: 개요를 GUI+CLI 병기로, `--model` 모델별 속도/품질 안내 표 추가(large-v3/medium/small/base·tiny), 검증 상태를 실사용(장시간·다화자)·단위 38건 반영으로 갱신.
- 문서 모순 제거: project-plan 범위·산출물·사용자를 GUI 포함으로 갱신, PLAN.md 상단에 "초기 설계 스냅숏 — 현재 기준은 docs/Decisionlog 참조" 배너 추가, CLAUDE.md 최종 산출물에 GUI 반영.
- implementation-plan 진행 상태를 전 단계 완료로 갱신(S0~S6, 실사용, T-001~T-007).

**변경 파일**
- README.md, docs/project-plan.md, docs/implementation-plan.md, PLAN.md, CLAUDE.md, Worklog.md

**검증**
- 단위 테스트 38/38. 문서 상호 참조(계약↔코드↔README↔로그) 육안 정합 점검. 실행 코드 변경 없음(문서 정리).

**판단 근거**
- CLAUDE.md 8절(문서 무모순)·README 실구현만 기재 원칙에 따라 초기 설계와 현재 상태의 모순을 배너·갱신으로 해소.

**결과**
- **S5 완료 → 프로젝트 계획 단계 전부 완료.** 도구는 GUI/CLI 양쪽으로 실사용 가능 상태.
- 남은 것은 선택적 개선(잡음/방언 표본 확대, 화자 수 지정 비교, exe 패키징 등)뿐이며 기능 결함은 없음.

### W-016 · 실사용 검증 — 장시간(32분) 다화자 예능 영상
**요청**
- 사용자가 GUI로 32분 예능 토크 영상을 화자 구분 포함 변환 후 "잘 된 것 같은데 확인 바람".

**수행 작업**
- 결과 md 검사: 길이 00:32:33, 문단 309개(60초 캡·화자 전환 분리 정상), 화자 1~12 감지, 진행자(화자 1)-게스트(화자 2) 문답 패턴이 라벨에 자연스럽게 반영됨을 확인.
- 로그 판독: 경고 0건(W-015 반영), 토큰 안내 없음(T-007 반영) — 두 수정의 실사용 확인.
- 한계 안내: 예능 특성상 화자 과분할 가능성(문단 수 적은 화자들). 실제 인원을 알면 GUI "화자 수" 지정 후 재변환 권장.
- validation-plan 갱신: 미검증이던 "장시간 30분+"와 "다화자 실사용" 처리 검증 완료로 이동. 화자 수 정답 대조는 사용자 판정 항목으로 남김.

**변경 파일**
- docs/validation-plan.md, Worklog.md

**검증**
- 위 결과 md 구조 통계로 확인(문단 수·화자 분포·발췌 대조). 처리 자체 exit 0.

**판단 근거**
- 실사용 파일이 미검증 범위 2건(장시간·다화자)의 자연 표본이 됨.

**결과**
- 완료. 남은 작업: S5(문서 최종 정리), (선택) 화자 수 지정 재변환 비교.

### W-015 · 실사용 후속 개선 — md 문단 캡(D-005) + 로그 경고 숨김
**요청**
- GUI 재시도 로그를 보여주며 "문제 없나?" 확인 요청.

**수행 작업**
- 판독: 오류 없음 — 토큰 안내가 사라졌고(T-007 해결 확인) 경고 3종은 상류 라이브러리의 정보성 안내(torch.load weights_only 예고, TF32 재현성, std 자유도). 화자 라벨 정상 부착 확인.
- 발견 1 — **md 벽글 문제**: 해당 영상 md가 7.5분 전체 문단 1개(편집 영상은 쉼이 잘려 "간격 ≥2초" 규칙이 발동 안 함; 무신사 영상은 8문단으로 대조 확인).
- 개선 1: 계약에 **문단 최대 60초 캡** 추가(D-005) → formatters.merge_paragraphs에 강제 분할 규칙 구현(md·txt --no-timestamps 공용).
- 개선 2: diarizer의 파이프라인 로드·실행 구간에서 상류 정보성 경고를 범위 한정으로 숨김(GUI 로그 소음 제거, 오류는 그대로 노출).

**변경 파일**
- docs/requirements-contract.md, Decisionlog.md(D-005), audio_to_text/formatters.py, audio_to_text/diarizer.py, tests/test_formatters.py, docs/validation-plan.md, Worklog.md

**검증**
- 단위 테스트 38/38(문단 캡 테스트 추가).
- E2E(같은 영상의 tests/ 사본, 환경변수 토큰 없음): 경고 라인 0건, 문단 8개(약 60초 간격), 화자 라벨 정상, exit 0.

**판단 근거**
- 실사용 피드백 기반 계약 보완(기준 변경은 코드보다 문서 먼저 — CLAUDE.md 5절).

**결과**
- 완료. 사용자 안내: GUI에서 "기존 결과 덮어쓰기" 체크 후 재변환하면 새 문단 구성이 적용됨.

### W-014 · GUI 토큰 미인식 수정 — 레지스트리 폴백 (T-007)
**요청**
- GUI 첫 실사용에서 "왜 토큰이 없다고 하지?" 문의.

**수행 작업**
- 원인 진단: 자식 프로세스는 부모의 환경변수 스냅숏을 상속 — 토큰 등록 이전에 시작된 개발 세션에서 띄운 GUI라 HF_TOKEN이 비어 있었음(T-007).
- 수정: diarizer에 `find_hf_token()` 도입(명시 인자 → 환경변수 → HKCU\Environment 레지스트리). 기존 "토큰 없음" 단위 테스트를 mock 기반으로 보정 + 토큰 탐색 테스트 4건 추가.
- 부수 확인: 이번 실사용으로 GUI 수동 검증 일부 완료(파일 추가→변환→저장, 미준비 안내 폴백 실동작).

**변경 파일**
- audio_to_text/diarizer.py, tests/test_diarizer.py, Troubleshootinglog.md(T-007), docs/validation-plan.md, Worklog.md

**검증**
- 단위 테스트 37/37 통과.
- 사용자 조건 재현: 환경변수에 HF_TOKEN 없는 셸에서 `--diarize` 실행 → 화자 1/2 라벨 정상 출력, 안내 메시지 없음(레지스트리 폴백 동작).
- 떠 있는 GUI는 재시작 불필요(변환 하위 프로세스가 수정된 코드를 새로 로드).

**판단 근거**
- 실행 경로별 환경 상속 차이를 코드가 흡수하는 것이 사용자 안내(재시작 요구)보다 근본적.

**결과**
- 해결 완료. 남은 작업: GUI 잔여 수동 확인([중지]·[폴더 열기]), S5.
